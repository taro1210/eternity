...
public class kadai3 {
	...
	public static void main(String[] args) {
		String[] novel = text.split("\\W");
		HashMap<String, Integer> novels = new HashMap<>();
		...
		for (String s : novel) {
			...
			if (novels.containsKey(s)) {
				// Mapに登録済み
				valueStr = novels.get(s) + 1;
			} else {
				// Mapに未登録
				valueStr = 1;
			}
			novels.put(s, valueStr);
		}
		for (java.util.Map.Entry<String, Integer> entry : novels.entrySet()) {
			if (entry.getValue() >= 3)
				System.out.printf("["+entry.getKey() + "] \t" + entry.getValue() + "回\r\n");
		}
	}
}